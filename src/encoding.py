...
def encoding(dataset_dir,encoding_file='encodings.csv', verbose=False,model='hog'):
	...
	for class_dir in os.listdir(dataset_dir):
		...
		for img_path in image_files_in_folder(os.path.join(dataset_dir, class_dir)):
			...
			# I'm trying to find faces rotating the image
			face_bounding_boxes=0
			rotated = np.zeros(image.shape)

			for i in (0,90,180,270):
				rotated = imutils.rotate(image,i)
				face_bounding_boxes = face_recognition.face_locations(rotated,model=model)
				if len(face_bounding_boxes) > 0:
					print('Found somebody when rotated at %d degrees'%(i))
					break
			...
			if len(face_bounding_boxes) != 1:
				...
				X.append(face_recognition.face_encodings(rotated, known_face_locations=face_bounding_boxes)[0])
				y.append(class_dir)
...
def main(argv):
	...
	encoding(DATASET_DIR, verbose=True,model=FACE_DETECTION_MODEL)
